...
contains
  ...
  subroutine test_interpolate_amr(&
       nDim, IsCartesian, IsPeriodicIn_D, nSample, UseGeneric, UseGhostCell)
    ...
    real, dimension(nDim):: &
         ...
         Coord_D, CoordPass_D, CoordCont_D,  &
         CoordInterpolated_D, CoordCorner_D, &
         CoordModulo_D, CellSize_D
  ...
  contains
    ...
    subroutine fix_coord(nDim, iBlock, CoordIn_D, CoordOut_D)
      ...
      real:: CellSize_D(nDim), CoordBlockMin_D(nDim), CoordBlockMax_D(nDim), &
           iDiscr_D(nDim)
      ...
      if(IsSpherical)then
         ...
         if(PhiAuxLo >= CoordBlockMin_D(Phi_) - CellSize_D(Phi_)*(1+cTol) &
              .and. PhiAuxLo &
              < CoordBlockMax_D(Phi_) + CellSize_D(Phi_)*(1+cTol))then
            PhiAux = PhiAuxLo
         elseif(PhiAuxHi >= CoordBlockMin_D(Phi_) - CellSize_D(Phi_)*(1+cTol) &
              .and. PhiAuxHi < CoordBlockMax_D(Phi_) &
              + CellSize_D(Phi_)*(1+cTol))then
            PhiAux = PhiAuxHi
         else
            RETURN
         end if
    ...
    subroutine check_interpolate_test(nDim, Coord_D, iBlockIn, &
      ...
      where(Coord_D < Coord_DGB(:,1,1,1,iBlockIn))
         iDiscr_D(1:nDim) = -1
      elsewhere(Coord_D >= &
           Coord_DGB(:,nCell_D(1),nCell_D(2),nCell_D(3),iBlockIn))
         iDiscr_D(1:nDim) =  1
      end where
    ...
    subroutine fill_nei_levels
      ...
      real:: Coord_D(nDim), CellSize_D(nDim), CoordCenter_D(nDim)
    ...
    subroutine fill_ghost_cells
      ...
      real:: Coord_D(nDim), CoordCorner_D(nDim), CellSize_D(nDim), &
           DcoordModulo_D(nDim)
      !------------------------------------------------------------------------
         ...
         do k = 1-nG, nCell_D(3) + nG
            do j = 1-nG, nCell_D(2) + nG
               do i = 1-nG, nCell_D(1) + nG
                  iCell_D = [i,j,k]
                  ! skip physical cells
                  if(all(iCell_D >= 1) .and. all(iCell_D <= nCell_D)) CYCLE
                  Coord_D = Coord_DGB(:,i,j,k,iBlock)
                  call find_test(nDim, Coord_D, &
                       iProc, iBlockNei, CoordCorner_D, CellSize_D, IsOut)
                  DcoordModulo_D = Coord_DGB(:,i,j,k,iBlock) &
                       - Coord_D - CoordCorner_D
                  where(DcoordModulo_D > CellSize_D .and. IsPeriodic_D(1:nDim))
                     DcoordModulo_D = DomainSize_D
                  elsewhere(DcoordModulo_D < -CellSize_D &
                       .and. IsPeriodic_D(1:nDim))
                     DcoordModulo_D = -DomainSize_D
                  elsewhere
                     ! DcoordModulo_D = 0.0
                  end where
                  if(IsOut) CYCLE
                  ! don't fill ghost cells for Coarse block if nei is Fine
                  if(  iTree_IB(Level_,iBlock   ) == Coarse_ .and.&
                       iTree_IB(Level_,iBlockNei) == Fine_) CYCLE
                  iCellIndexNei_D = 1
                  iCellIndexNei_D(1:nDim) = &
                       nint(0.3 + Coord_D(1:nDim)/CellSize_D(1:nDim))
                  Var_GB(i,j,k,iBlock) = &
                       Var_GB(iCellIndexNei_D(1), &
                       iCellIndexNei_D(2),iCellIndexNei_D(3),iBlockNei)
                  Coord_DGB(:,i,j,k,iBlock) = &
                       Coord_DGB(:,iCellIndexNei_D(1), &
                       iCellIndexNei_D(2),iCellIndexNei_D(3),iBlockNei)
                  call fix_coord(nDim, iBlock, Coord_DGB(:,i,j,k,iBlock), &
                       Coord_D)
  ...
  !============================================================================
  subroutine find_test( &
       nDim, Coord_D, iProc, iBlock, CoordCorner_D, CellSize_D, IsOut)
    ...
    integer:: iShift_D(3)
    integer, parameter:: iChildFromShift_III(0:1,0:1,0:1) = reshape( &
         [1, 2, 3, 4, 5, 6, 7, 8],[2, 2, 2])
    logical:: IsAboveCenter_D(nDim)
