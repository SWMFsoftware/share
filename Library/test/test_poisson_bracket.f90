...
contains
  ...
  subroutine test_multipoisson_bracket(TimeOut)
    ...
    real,parameter :: &
         ProtonmassInGeV=cProtonmass*clightspeed**2/&
         (1e9*cElectroncharge)                                 ! proton mass in unit of GeV/c^2
    ...
    real :: VDFOutput_II(nQ,nR)                                ! vdf output
    ...
    real :: DeltaSOverB_C(nQ), DDeltaSOverBDt_C(nQ), InvB_C(nQ), &
         bDuDt_C(nQ), DLnBDeltaSSquaredDt_C(nQ)                ! The data calculated directly from RawData
    ...
    GapData = 0.0
    do while(nint(GapData) /= iLagrIDSkipped)
       read(13,*)GapData
    end do
    do iQ = 1, nQ
  ...
  contains
    ...
    subroutine calc_initial_data(&
         RawData1_II, RawData2_II,Time,DeltaSOverB_C,DDeltaSOverBDt_C, &
         InvB_C,bDuDt_C,DLnBDeltaSSquaredDt_C)

      ! calculate data from the input files

      real, optional, intent(inout) :: DeltaSOverB_C(nQ), DDeltaSOverBDt_C(nQ), InvB_C(nQ), bDuDt_C(nQ), DLnBdeltaSSquaredDt_C(nQ)
      real, intent(in)    :: RawData1_II(nQ,nVar), RawData2_II(nQ,nVar)
      real, intent(in)    :: Time
      real :: DeltaSOverBOld_C(nQ), InvBOld_C(nQ), LnBDeltaSSquaredOld_C(nQ)!\deltas/b, 1/(2B), ln(B\deltas^2) at Old time
      real :: DeltaSOverBNew_C(nQ), InvBNew_C(nQ), LnBDeltaSSquaredNew_C(nQ)!\deltas/b, 1/(2B), ln(B\deltas^2) at New time
      real :: B_C(nQ,3)! magnetic field B at cell center
    ...
    subroutine calc_hamiltonian_3(DeltaSOverB_C,bDuDt_C,DLnBDeltaSSquaredDt_C,Hamiltonian3_N,DeltaP3_I,DeltaLnP3,LnP3min,LnP3max)
      ...
      real, intent(in) :: DeltaP3_I(nR), DeltaLnP3, LnP3min, LnP3max
      real, intent(in) :: DeltaSOverB_C(nQ), bDuDt_C(nQ), DLnBDeltaSSquaredDt_C(nQ)
...
module ModTestPoissonBracketAndScatter
  ...
  ! number of vatiales in each line of the input files
  integer,parameter :: nVar = 14
...
contains
  ...
  subroutine test_scatter(TimeOut)
    real,intent(in) :: TimeOut
    ! Misc:
    integer,parameter :: Diffuornot=0                         ! whether using dissusive approximation or not
    real           :: DtFixed = 0.03                          ! optional fixed time step
    real           :: Time=0.0, Dt=0.0                        ! simulation time, time step
    real,parameter :: ProtonmassInGeV = &
         cProtonmass*clightspeed**2/(1e9*cElectroncharge)     ! proton mass in unit of GeV/c^2
    real,parameter :: ParticleRange1=2.0, ParticleRange2=4.0  ! parameter of the place of the initial particles
    integer,parameter :: nQ = 130,  nP = 20,  nR = 20         ! number of grid of s_L, \mu, ln(p^3/3) axis
    ...
    real :: Volume_G(0:nQ+1, 0:nP+1, 0:nR+1)                  ! control volume
    real :: VDF_G(-1:nQ+2, -1:nP+2, -1:nR+2)                  ! vdf
    real :: VDF_Final(nQ,nR)                                  ! vdf for output
    real :: Hamiltonian2_N(-1:nQ+1,-1:nP+1,0:nR+1)            ! the Poisson bracket with regard to the first and second vars
    ...
    real :: InvB_C(nQ)                                        ! 1/(2B)
    real :: DeltaSOverB_C(nQ)                                 ! Cell centered Delta S over B
    real :: DeltaS0_I(nQ-1)                                   ! Distance between the cell centers
    real :: LambdaMuMu_II(nQ,nR)                              ! The data that is calculated directly from RawData
    real :: tOutput                                           ! Time of each output file
    ...
    ParticleEnergyMin = 1.0e-3       ! Min energy in unit of Gev
    ParticleEnergyMax = 1.0          ! Max energy in unit of Gev
    ! give the min value of Ln(P^3/3). We set the minimum of energy to be 1MeV, then Ln(p^3/3) can be calculated
    LnP3min = log(sqrt((ParticleEnergyMin+ProtonmassInGeV)**2 - ProtonmassInGeV**2)**3.0/3.0)
    ! give the max value of Ln(P^3/3). We set the maximum of energy to be 1GeV, then Ln(p^3/3) can be calculated
    LnP3max = log(sqrt((ParticleEnergyMax+ProtonmassInGeV)**2-ProtonmassInGeV**2)**3.0/3.0)
    ...
    do iR = 1, nR
       ! Assume the momentum of particls to be power distribution: f ~ p^-5
       VDFValue = max(exp(-5.0/3.0*(LnP3min+iR*DeltaLnP3)), 1e-7)
       do iQ = 1, nQ
          if (iQ<=ParticleRange1)then
    ...
    ! Calculate the position for ln(p^3/3) axis: note that the energy is in the unit of KeV now
    do iR=1,nR
       Energy_I(iR) = 6 + log10(sqrt((3*exp(LnP3min + (iR - 0.5)*DeltaLnP3))**(2.0/3.0) &
            + ProtonmassInGeV**2) - ProtonmassInGeV)
    end do
    ...
    do IndexofFile=1,nOutputFile
       ...
       do iQ = 1, nQ
          HelioDist_I(iQ) = sqrt(RawData1_II(iQ,x_)**2 + RawData1_II(iQ,y_)**2 + RawData1_II(iQ,z_)**2) &
               * clightspeed / rSun
       end do
  ...
  contains
    ...
    subroutine calc_initial_data
      ...
      do iR = 1, nR
         Momentum = (3.0*exp(LnP3min + (iR - 0.5)*DeltaLnP3))**(1.0/3.0)
         do iQ = 1, nQ
            LambdaMuMu_II(iQ, iR) = CoeffMu*MagneticOverWave_C(iQ)*LarmorRadius13_C(iQ) &
                 *Lmax_C(iQ)**(2.0/3.0)*Momentum**(1.0/3.0)
         end do
      end do
      ...
      do iQ=0,nQ
         do iP=0,nP
            do iR=1,nR
               ...
               ! Note that light speed is the unit of speed in our code,
               ! so we do not need to multiply a c^2 in the following expression
