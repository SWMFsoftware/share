...
class ReadParam{
 public:
  std::stringstream ss;
  bool isProc0; // Bad name! Use isVerbose.
 public:
  ReadParam(){ isProc0=true;}
  ~ReadParam(){}
  ...
  template <class T>
    void read_var(std::string description, T & var) {    

    if(ss.good()) ss >> var; // If ss is empty, ss.good() is still true.
	...
	return;
      ...
      std::cout << "Type T: can not find input parameter. Abort!" << std::endl;
      std::abort();
    }
  }
  //=======================================================
  ...
  inline void read_var(std::string description, std::string & var) {
	...
	return;
      ...
      std::cout << "String: can not find input parameter. Abort!" << std::endl;
      std::abort();
    }
  }
  //=========================================
