...
if($Help or not @source){
    print "
This script can be used to improve formatting of Fortran code
by replacing obsolete notation, fixing separator lines etc.
In addition the script can add calls to subroutines test_start 
and test_stop from BATL_test to improve debugging capabilities.
...
-e=PATTERN   Do not add tests to files and methods matching pattern.
...
-method=TYPE are 'subroutine', 'function', 'all' or 'none' 
             Only the first character matters. Default is 'none'.
...
Improve formatting of Fortran code

     ./$SCRIPT *.f90

Add tests to subroutines at default level and fix code in all Fortran 90 files:

     ./$SCRIPT -method=subroutine *.f90
